...
test("generated icons preserve the frozen public alpha asset", () => {
  ...
  const asset = fs.readFileSync(path.join(simulatorRoot, "web", "assets", "x3-icons.js"));
  assert.equal(
    crypto.createHash("sha256").update(asset).digest("hex"),
    "d577a3a2b85ebe7f5bc444afc76d1ee6af3ddb4dfd0555751f343a90b3ce82fb"
  );
});
