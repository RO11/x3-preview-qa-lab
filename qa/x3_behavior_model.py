...
def utc_plus_10_local_day(utc_epoch: int) -> int:
    if utc_epoch < 0:
...
def next_wake_seconds(
    ...
    *,
    ...
    local_now = now_utc.astimezone(timezone(timedelta(hours=10)))
    choices: list[tuple[int, int, int]] = []
    for hour, minute in windows:
        target = local_now.replace(hour=hour, minute=minute, second=0, microsecond=0)
        if target <= local_now:
            target += timedelta(days=1)
        choices.append((int((target - local_now).total_seconds()), hour, minute))
    return min(choices)
